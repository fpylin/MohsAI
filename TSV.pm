...
###############################################################################
sub new {
    my ($class, $filename, $field_ref, $params) = @_;
    ...
    if ( defined $params and exists $$params{debug} ) {
...
sub load {
    ...
    print STDERR "$filename: " if ! exists $self->{debug} or $self->{debug} == 0 ;
...
sub import_column {
	...
	my $field = shift;
	my @data  = @{ $_[0] } ;
	my $insert_before = $_[1];
	
	if ( ! defined $insert_before ) {
        push @{ $self->{'fields'} }, $field;
    } else {
        my @fields = @{$self->{'fields'}};
        my ($position) = grep { $fields[$_] eq $insert_before } ( 0 .. $#fields );
        splice(@{ $self->{'fields'} }, $position, 0, $field);  
    }
	my $N = undef;
...
#     print STDERR "TSV->get(): ".join(", ", @fields)."\n";
    ...
    for my $row ( @{ $self->{'data'} } ) {
        my %a = %{ $row };
        my @z ; warn "TSV->get(): Row $rowno: Undefined fields: ".join(", ", @z)."\n" if ( @z = grep { ! defined $a{$_} } @fields );
...
#     print STDERR join(" ", "TSV::is_numeric", $field, scalar(@n_match)."/".scalar(@levels), @levels)."\n";
...
sub grep_rows{ 
    ...
    my @a = grep { our $row = $_; local $_ = $row; $code_ref->($_) } @{ $self->{'data'} };
...
sub index_by{
    ...
    my $line_no = 1;
    for my $row ( @{ $self->{'data'} } ) {
		warn "Line $line_no: Field $$row{$field} already exists\n" if $warning and exists $a{ $$row{$field} };
        warn "Line $line_no: Key $field is undefined\n" if $warning and ! defined $$row{$field} ;
        next if ! defined $$row{$field} ;
		$a{ $$row{$field} } = $row;
		++$line_no;
    }
    return %a;
...
sub index_hash_by_flat{
    my $self = shift;
    my $value = pop;
    my @fields = @_; # composite index allowed, separated by \t
    my %a ;
    my $line_no = 1;
    
    for my $row ( @{ $self->{'data'} } ) {
        my $key = join("||||", ( map { $$row{$_} // 'NA' } @fields ) );
		$a{ $key } = $$row{ $value };
    }
    return %a;
}
...
sub index_by_strata {
    ...
    my %a ;
    for my $row ( @{ $self->{'data'} } ) {
		if ( defined $field2 and defined $field3 ) {
			push @{ $a{ $$row{$field} }{ $$row{$field2} }{ $$row{$field3} } }, $row;
		} elsif ( defined $field2 ) { 
			push @{ $a{ $$row{$field} }{ $$row{$field2} } }, $row;
		} else {
			push @{ $a{ $$row{$field} } }, $row;
		}
    }
    return %a;
}
...
sub replace {
    ...
    my $repl_table = shift;
    for my $row ( @{ $self->{'data'} } ) {
        $$row{$field} = $$repl_table{ $$row{$field} } if exists $$repl_table{ $$row{$field} } ; 
    }
    return $self;
}
...
sub left_join {
    ...
    my $field = shift;
    my $tsv2 = shift;
    my @fields_to_import = @_;
    my %tsv2_by_hash = $tsv2->index_by($field);
    my @fields_to_add = scalar(@fields_to_import) ? @fields_to_import : grep { $_ ne $field } @{ $tsv2->{fields} } ;
    
    # Add the fields from $tsv2 to the current object
    push @{ $self->{fields} }, @fields_to_add;

    # Loop through each row in the current object
    foreach my $row (@{ $self->{data} }) {
        my $key = $row->{$field};

        # If the key exists in $tsv2, copy the corresponding values
        if (exists $tsv2_by_hash{$key}) {
            my $tsv2_row = $tsv2_by_hash{$key};

            # Copy the values from $tsv2 to the current object
            foreach my $field_to_add (@fields_to_add) {
                $row->{$field_to_add} = $tsv2_row->{$field_to_add};
            }
        }
    }
    return $self;
